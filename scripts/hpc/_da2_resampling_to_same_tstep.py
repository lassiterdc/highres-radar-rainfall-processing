...
# final_chunking_dict = dict(time = tsteps_per_hr*1, latitude = 5, longitude = 5) | killed
# final_chunking_dict = dict(time = tsteps_per_hr*1, latitude = 5, longitude = 5) | killed
# final_chunking_dict = dict(time = tsteps_per_hr*3, latitude = 1, longitude = 1) | killed
# final_chunking_dict = dict(time = tsteps_per_hr*3, latitude = 3, longitude = 3) | killed
# final_chunking_dict = dict(time = tsteps_per_hr*3, latitude = 5, longitude = 5) | killed
# final_chunking_dict = dict(time = tsteps_per_hr*6, latitude = 5, longitude = 5) | killed
# final_chunking_dict = dict(time = tsteps_per_hr*6, latitude = 3, longitude = 3) | killed
# final_chunking_dict = dict(time = tsteps_per_hr*6, latitude = 1, longitude = 1) | killed
# final_chunking_dict = dict(time = tsteps_per_day/2, latitude = 5, longitude = 5)| killed
# final_chunking_dict = dict(time = tsteps_per_day/2, latitude = 3, longitude = 3)| killed
# final_chunking_dict = dict(time = tsteps_per_day/2, latitude = 1, longitude = 1)| killed
...
def bias_correct_and_fill_mrms(ds_mrms, ds_ref, lst_tmp_files_to_delete, 
    ...
    # bm_time = time.time()
    # xds_mrms_hourly_to_ref.chunk(dic_chunks_ref).to_zarr(tmp_zarr, mode = "w", encoding = define_zarr_compression(xds_mrms_hourly_to_ref), consolidated=True)
    # xds_mrms_hourly_to_ref = xr.open_zarr(store=tmp_zarr).chunk(dic_chunks_ref)
    ...
    # print(f"Time to export xds_mrms_hourly_correction_factor_fulres to zarr (min): {((time.time() - bm_time)/60):.2f} | total script runtime (min): {((time.time() - start_time)/60):.2f}")
    #
    #
