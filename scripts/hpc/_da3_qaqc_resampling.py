...
df = pd.read_csv(fldr_out_nc+"_da3_resampling_performance.csv")
...
for year in df_yearmonths.year.unique():
    ...
    bm_time2 = time.time()
    ds_qaqc_year = xr.open_dataset(fl_nc_qaqc_out, engine="h5netcdf", chunks={"date":"auto"})
...
bm_time = time.time()
ds_qaqc_all = xr.open_mfdataset(lst_ncs_year, engine = "h5netcdf", chunks=dict(date = "auto"))
d_encoding = {}
...
bm_time = time.time()
ds_qaqc_all_st4_res = xr.open_mfdataset(lst_ncs_year_st4_res, engine = "h5netcdf", chunks = dict(date = "auto"))
d_encoding = {}
for da_name in ds_qaqc_all_st4_res.data_vars:
    d_encoding[da_name] = {"zlib":True}

ds_qaqc_all_st4_res.to_netcdf(fl_nc_qaqc_out_all_st4_res, encoding=d_encoding, engine="h5netcdf")
time_elapsed_min = round((time.time() - bm_time) / 60, 2)
print("Exported netcdf qaqc file for entire dataset. Time to export: {}.".format(time_elapsed_min))
