...
for i, f in enumerate(files):
    ...
    if ".grib2" in f:
        if i == 0: # do everything for the grib file in a single iteration of the loop
            ...
            ds_comb["longitude"] = ds_comb["longitude"].astype(np.float32)
            ds_comb["latitude"] = ds_comb["latitude"].astype(np.float32)
            # define attributes
            ds_comb.attrs['rainrate_units'] = "mm_per_hour"
            ds_comb.attrs['time_units'] = "proceeding"
            ds_comb.attrs['time_zone'] = "UTC"
            ds_comb.attrs['longitude_units'] = "degrees_east"
            ds_comb.attrs['latitude_units'] = "degrees_north"
            ds_comb.attrs['gridcell_feature_represented_by_coordinate'] = "center"
            ds_comb.attrs['data_source'] = {"origin":origin,
            ...
            in 2022 or 2023."""}
            ds_comb.attrs['warnings'] = dic_warnings
        else:
            continue
        ...
        # ds.attrs['data_source'] = {"origin":"iowa_state_mesonet_quantized_pngs_to_netcdfs_dataset_20130801_to_20141231",
...
def check_for_center(ds_comb, s_nw_corner_lat, s_nw_corner_lon,
                    s_se_corner_lat, s_se_corner_lon):
...
def process_for_rainyday(ds_comb, v_lats_most_recent, v_lons_most_recent, lst_problems):
    ...
    s_ds_minus_center_nw_corner_lat, s_ds_minus_center_nw_corner_lon, s_ds_minus_center_se_corner_lat, s_ds_minus_center_se_corner_lon = \
        check_for_center(ds_comb, s_nw_corner_lat, s_nw_corner_lon,s_se_corner_lat, s_se_corner_lon)
    #
    ...
    ds_comb.attrs["data_source"] = str(ds_comb.attrs["data_source"])
    ...
    #
    # make sure there are no boolean attributes
    for at in ds_comb.attrs:
        if ds_comb.attrs[at] == True:
            ds_comb.attrs[at] = "True"
