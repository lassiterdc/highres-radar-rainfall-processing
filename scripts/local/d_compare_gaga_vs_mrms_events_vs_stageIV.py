...
f_nc_24h_atgages, f_nc_atgages, fldr_stageIV_data, f_csv_mrms_and_gage_events, f_csv_stageiv_and_gage_events, fldr_out_plots, f_shp_coast, f_shp_subcatchments, f_shp_gages = return_d_filepaths()
...
# stage_iv =  xr.open_mfdataset(fldr_stageIV_data.format("*", "*"))
...
for yr in df_gage_and_mrms.time.dt.year.unique():
    if exclude_2012_2013_and_2014 == True:
        if yr in [2012, 2013, 2014]:
            ...
            lst_yrs.append(yr)
    for f in glob(fldr_stageIV_data.format(yr, "*")):
...
def plot_compare_stageIV_and_mrms(gage_totals, tseries, cbar_high, title, fname, gds_subs_trns=gds_subs_trns, gds_gages_trns=gds_gages_trns, gds_coast_trns=gds_coast_trns):
    fig, axes = plt.subplots(1,2, figsize = [figwidth*2, figheight], subplot_kw=dict(projection=proj), dpi=300)
    ######### PLOT MRMS #########
    gage_totals_renamed = gage_totals.reset_index().rename(columns={"gage_id":"MONITORING"})
    gds_gages_trns_with_rainfall = gds_gages_trns.merge(gage_totals_renamed, on="MONITORING")
    ...
    for x, y, label in zip(gds_gages_trns_with_rainfall.geometry.x, gds_gages_trns_with_rainfall.geometry.y, gds_gages_trns_with_rainfall.MONITORING):
        ...
        if label == "MMPS-140":
            xytext=(-40, 5)
        axes[0].annotate(label, xy=(x, y), xytext=xytext, 
            textcoords="offset points", color="white", fontsize="x-small",
            fontweight='demibold')
    ...
    im = da_event_total_inches.plot.pcolormesh(x="longitude", y="latitude", ax=axes[1], 
                        vmin=0, vmax=np.ceil(float(cbar_high)), cmap='gist_rainbow', add_colorbar = False)
    
    cbar_ax = fig.add_axes([1, 0.1, 0.01, 0.7])
    ...
    for x, y, label in zip(gds_gages_trns_with_rainfall.geometry.x, gds_gages_trns_with_rainfall.geometry.y, gds_gages_trns_with_rainfall.MONITORING):
        ...
        if label == "MMPS-140":
            xytext=(-40, 5)
        axes[1].annotate(label, xy=(x, y), xytext=xytext, 
            textcoords="offset points", color="white", fontsize="x-small",
            fontweight='demibold')
...
# for e_id in event_ids[0:1]:
    ...
    title = "Event {}: {} to {}".format(e_id, str_start_datetime, str_end_datetime)
    fname = fldr + "{}_e_id_{}.png".format(event_index, e_id)
    # try:
    #     xlim, ylim = plot_day_of_mrms_data(gage_totals, tseries, cbar_high_in, title, fname)
    # except:
    #     problem_events.append(title)

    plot_compare_stageIV_and_mrms(gage_totals, tseries, cbar_high_in, title, fname)

#%% plot the stage IV data with the gage data
...
# try:
#     shutil.rmtree(fldr_out_plots+"stage_iv")
# except:
#     pass
# try:
#     os.mkdir(fldr_out_plots+"stage_iv")
# except:
#     pass
...
#     title = "Event {}: {} to {}".format(e_id, str_start_datetime, str_end_datetime)
#     fname = fldr_out_plots + "stage_iv/{}_e_id_{}.png".format(event_index, e_id)
#     # try:
#     #     plot_day_of_stageIV_data(gage_totals, tseries, cbar_high_in, title, fname)
#     # except:
#     #     problem_events.append(title)
#     plot_day_of_stageIV_data(gage_totals, tseries, cbar_high_in, title, fname, xlim, ylim)
